# Interactive Refinement Workflow Δ
...
```typescript
interface InteractiveRefinementInput {
  userInput: string;           // User's input for this turn
  sessionId?: string;          // Optional: resume existing session
  database: string;            // Database context
}

interface InteractiveRefinementOutput extends SqlGenerationOutput {
  intent: 'new_query' | 'refinement';
  turnNumber: number;
  sessionId: string;
  conversationContext?: ConversationTurn[];
  refinementSummary?: string; // Only for refinements
}

interface TurnResult {
  success: boolean;
  result?: InteractiveRefinementOutput;
  error?: {
    message: string;
    canRetry: boolean;
  };
...
```typescript
async function handleTurn(input: InteractiveRefinementInput): Promise<TurnResult> {
  try {
    // 1. Detect intent
    const intent = dialogManager.detectIntent(input.userInput);
    ...
    if (intent === 'new_query') {
      ...
      const context = dialogManager.getContext();
      result = await sqlRefinementWorkflow.refine({
    ...
    dialogManager.addTurn(input.userInput, intent, result);

    // 4. Enrich result
    const enriched = enrichResult(result, intent, dialogManager.getTurnCount());
...
**Required:**
- DialogManager (new agent)
- SqlGenerationWorkflow (existing)
- SqlRefinementWorkflow (existing)

**Optional:**
- Logger for conversation analytics
...
```typescript
const CONFIG = {
  sessionTimeout: 3600000,       // 1 hour (milliseconds)
  enableConversationLogging: false,
  maxTurnLatency: 3000,          // 3 seconds (p95)
...
|------|-------------|----------|
| `INTENT_DETECTION_FAILED` | Could not determine intent | Default to new_query |
| `GENERATION_FAILED` | Query generation failed | Retry with user |
| `REFINEMENT_FAILED` | Query refinement failed | Retry with user |
| `STATE_SYNC_FAILED` | Conversation state corruption | Reset conversation |
...
```typescript
// In interactive mode command handler
const workflow = new InteractiveRefinementWorkflow(
  dialogManager,
  sqlGenerationWorkflow,
  sqlRefinementWorkflow
);

rl.on('line', async (line) => {
  const result = await workflow.handleTurn({
    userInput: line,
...
2. **Undo/redo:** Revert to previous turn
3. **Branching:** Explore alternative refinements
4. **Analytics:** Track conversation patterns, refinement success rates
5. **Multi-database:** Switch database mid-conversation
