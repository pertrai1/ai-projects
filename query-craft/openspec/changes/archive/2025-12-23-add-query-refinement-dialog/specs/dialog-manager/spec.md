...
interface ConversationTurn {
  id: string;                    // Unique turn identifier (UUID)
  timestamp: Date;               // When turn occurred
  userInput: string;             // User's input text
  intent: 'new_query' | 'refinement';
  result?: SqlGenerationOutput;  // Query generation/refinement result
}

interface ConversationState {
  sessionId: string;             // Session identifier (UUID)
  database: string;              // Database context for session
  turns: ConversationTurn[];     // Conversation history (max 10)
  currentQuery?: string;         // Last successful query
  currentResult?: SqlGenerationOutput;
}

interface ConversationContext {
  originalQuestion: string;      // First turn input
  currentQuery: string;          // Last query
  currentResult: SqlGenerationOutput;
  previousTurns: ConversationTurn[];
}

interface DialogManagerInput {
  sessionId?: string;            // Optional: resume existing session
  database: string;              // Database context
}

interface DialogManagerOutput {
  sessionId: string;
  intent: 'new_query' | 'refinement';
  confidence: 'high' | 'medium' | 'low';
  context?: ConversationContext; // Only for refinement intent
}
```
...
**Priority Order:**
1. First turn → always 'new_query' (high confidence)
2. Explicit commands (/new, /clear) → 'new_query' (high confidence)
3. Refinement keywords + previous query → 'refinement' (high confidence)
4. New query keywords + no refinement → 'new_query' (high confidence)
5. Short input (≤5 words) + previous query → 'refinement' (medium confidence)
6. Ambiguous → default based on previous query existence (low confidence)
...
const REFINEMENT_KEYWORDS = [
  ...
  'exclude', 'without', 'except', 'not'
];

const REFINEMENT_PHRASES = [
  'too many', 'too few', 'wrong', 'missing',
  'show x too', 'include x', 'exclude x'
];

const NEW_QUERY_KEYWORDS = [
  'show', 'find', 'get', 'list', 'what', 'which',
  'who', 'how many', 'count', 'select', 'display'
];

const RESET_PATTERNS = [
  '/new', 'new query', 'start over', 'reset', 'begin'
];
```
...
const CONFIG = {
  ...
  shortInputThreshold: 5,        // Words for "short" input
  enableLLMIntent: false,        // Use LLM for intent (future)
  logAmbiguousCases: true        // Log for analysis
};
```
...
| Condition | Behavior |
|-----------|----------|
| Empty user input | Treat as 'new_query', confidence 'low' |
| No previous query for refinement | Fallback to 'new_query' |
| State corruption | Reset state, log error |
| Memory limit exceeded | Prune history, continue |
...
1. **LLM-based intent detection:** Hybrid mode with rule-based fallback
2. **Conversation persistence:** Save/load conversation from file/DB
3. **Multi-database sessions:** Switch database within conversation
4. **Conversation analytics:** Track intent accuracy, refinement patterns
