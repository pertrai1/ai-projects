...
export interface TestQuery {
  query: string;
  intent: string; // ARCHITECTURE, IMPLEMENTATION, etc.
  expectedFunctions: string[]; // Functions we expect to find
...
export interface RetrievalMetrics {
  precision: number; // Of retrieved results, how many were relevant?
  recall: number; // Of relevant results, how many did we retrieve?
...
export interface ChunkingExperiment {
  ...
  statistics: {
    totalChunks: number;
    avgChunkSize: number;
    tokenEfficiency: number; // Chunks per 1000 tokens
  };
  retrieval: {
 ...
 */
export async function runChunkingExperiments(
  codebaseDir: string,
  testQueries: TestQuery[],
): Promise<ChunkingExperiment[]> {
  ...
  const experiments: ChunkingExperiment[] = [];

  // Test each strategy
  const strategies = [
    { name: "fixed", config: { strategy: "fixed" as const, maxChars: 500 } },
    {
      name: "semantic",
      config: { strategy: "semantic" as const, maxTokens: 1000 },
    },
    {
      name: "semantic-lookahead",
      config: { strategy: "semantic-with-lookahead" as const, maxTokens: 1000 },
    },
  ];
  ...
  for (const { name, config } of strategies) {
    ...
    try {
      ...
      // Aggregate metrics
      const avgMetrics: RetrievalMetrics = {
        precision:
          queryResults.reduce((sum, m) => sum + m.precision, 0) /
          queryResults.length,
        recall:
          queryResults.reduce((sum, m) => sum + m.recall, 0) /
          queryResults.length,
        mrr:
          queryResults.reduce((sum, m) => sum + m.mrr, 0) / queryResults.length,
        avgRelevanceScore:
          queryResults.reduce((sum, m) => sum + m.avgRelevanceScore, 0) /
          queryResults.length,
      };

      // Token efficiency
      const tokenEfficiency =
        (chunkResult.chunks.length * 1000) /
        (chunkResult.statistics.totalTokens || 1);

      // Create tradeoff analysis
      const tradeoffs = analyzeTradeoffs(
        name,
        chunkResult.statistics,
        avgMetrics,
        totalSearchTime / testQueries.length,
      );
...
function calculateQueryMetrics(
  ...
  const retrievedFunctions = new Set<string>();
  ...
  const relevantRetrieved = Array.from(retrievedFunctionSet).filter((f) =>
    expectedFunctionSet.has(f),
  ).length;
  const precision =
    retrievedFunctionSet.size > 0
      ? relevantRetrieved / retrievedFunctionSet.size
      : 0;

  // Recall: of relevant, how many did we retrieve?
  const recall =
    expectedFunctionSet.size > 0
  ...
  const avgRelevanceScore =
    retrievedChunks.length > 0
      ? retrievedChunks.reduce(
          (sum: number, c: any) => sum + (c.relevanceScore || 0),
          0,
 ...
 */
export function getExampleTestQueries(): TestQuery[] {
  return [
    {
