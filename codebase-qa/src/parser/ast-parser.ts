...
import { FileStructure, CodeASTNode } from '../types/index.js';
...
export class ASTParser {
   ...
   */
  private extractFunctions(lines: string[]): CodeASTNode[] {
    const functions: CodeASTNode[] = [];
   ...
   */
  private extractClasses(lines: string[]): CodeASTNode[] {
    const classes: CodeASTNode[] = [];
   ...
   */
  private extractInterfaces(lines: string[]): CodeASTNode[] {
    const interfaces: CodeASTNode[] = [];
  ...
  getDocumentation(lines: string[], nodeStartLine: number): string | undefined {
    if (nodeStartLine <= 1) return undefined;
 ...
 */
export function parseCodebaseStructure(): Promise<Map<string, FileStructure>> {
