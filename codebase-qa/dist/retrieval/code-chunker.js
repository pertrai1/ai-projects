...
export class SemanticChunker {
    ...
    chunkFile(filePath, content) {
        ...
        for (const node of structure.nodes) {
            ...
            if (estimateTokenCount(chunkContent) > this.maxTokens) {
                ...
                const subChunks = this.splitLargeNode(filePath, nodeLines, node.startLine, importText);
     ...
     */
    splitLargeNode(filePath, nodeLines, nodeStartLine, importText) {
        const chunks = [];
        let currentChunk = [];
...
export class FixedChunker {
    constructor(chunkSize = 500, overlapSize = 100) {
 ...
 */
export async function chunkCodebase(files, config) {
    let chunker;
    ...
    const statistics = {
        ...
        avgChunkSize: Math.round(allChunks.reduce((sum, c) => sum + c.content.length, 0) / allChunks.length),
        ...
        avgTokensPerChunk: Math.round(tokenCounts.reduce((a, b) => a + b, 0) / tokenCounts.length),
    };
    return {
        chunks: allChunks,
